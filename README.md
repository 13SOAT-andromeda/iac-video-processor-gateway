...
## 10. Decisões registradas (specs)

As decisões de arquitetura desta borda estão versionadas em [`docs/superpowers/specs/2026-07-11-gateway-design.md`](docs/superpowers/specs/2026-07-11-gateway-design.md):

- backend misto Lambda (auth) + EKS/ALB (domínio) num único HTTP API;
- catch-all por serviço em vez de rota por verbo;
- tag exclusiva do ALB e Ingress centralizado (correção do bug de múltiplos ALBs do tech-challenge anterior);
- zero IAM role neste repo (compatibilidade AWS Academy/LabRole);
- pontos em aberto: rate limiting/throttling nas rotas públicas de `/auth/*` e HTTPS ponta-a-ponta até o pod (hoje: HTTP interno na porta 80).
